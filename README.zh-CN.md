...
Agent Island 当前公开可验证的安装路径是本地源码构建。这个 fork 还没有可验证的 DMG/Homebrew 安装入口。
...
macOS 13+，通用二进制（Apple 芯片 + Intel）。`./scripts/verify.sh` 会构建并短暂启动一次应用做 smoke test；`open build/AgentIsland.app` 用来正常启动本地构建。这个本地源码构建默认 `SU_FEED_URL` 为空，因此不会自动更新。
